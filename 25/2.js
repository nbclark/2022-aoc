...
(function () {
  ...
  for (let i = 0; i < 10000; ++i) {
    ...
    b = bs;
    const exists = map[JSON.stringify(bs)];
    if (exists) {
      console.log({ i });
      process.exit(0);
    }
    map[JSON.stringify(bs)] = i;
...
function calculatePath(
  ...
  let queue = [{ x: start.x, y: start.y, bs: blizzards, count: startIndex }];
  while (queue.length) {
    const nextQueue = [];
    const seen = {};
...
const firstCount = calculatePath(1);
const secondCount = calculatePath(
  firstCount,
  { x: grid[0].length - 2, y: grid.length - 1 },
  { x: 1, y: 0 }
);
const thirdCount = calculatePath(secondCount);
