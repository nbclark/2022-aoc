...
const xMin = Math.min(sand.x, sortedByX[0].x);
const xMax = Math.max(sand.x, sortedByX[sortedByX.length - 1].x);
const yMin = Math.min(sand.y, sortedByY[0].y);
const yMax = Math.max(sand.y, sortedByY[sortedByX.length - 1].y);
...
while (count++ < 300000) {
  try {
    // Move some sand
    let pos = { x: sand.x - xMin, y: sand.y + 1 - yMin };
    do {
